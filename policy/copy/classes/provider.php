...
class provider implements policy_interface {
    ...
    public static function add_admin_settings(admin_settingpage $settings): void {
        ...
        $settings->add(
            new admin_setting_confightmleditor(
                ...
                get_string("message_default_text", "proctoringpolicy_copy")
            )
        );
    }
    ...
    public static function add_module_form(moodleform_mod $formwrapper, MoodleQuickForm $mform, int $cmid): void {
        ...
        $mform->addElement("html", "<fieldset class='proctoring-block'><legend>" . get_string("legend", "proctoringpolicy_copy") . "</legend>");

        $mform->addElement("advcheckbox", "kopere_policy_copy_enabled", get_string("enabled_cm", "proctoringpolicy_copy"));
        $mform->setType("kopere_policy_copy_enabled", PARAM_INT);
        $mform->setDefault("kopere_policy_copy_enabled", 0);
    ...
    public static function get_js_config(int $cmid, int $attemptid): array {
        $enabled = cm_config::get("copy", "enabled", $cmid, 0);
        if (!$enabled) {
            return [];
        }
