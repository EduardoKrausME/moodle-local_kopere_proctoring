...
class provider implements policy_interface {
    ...
    public static function add_module_form(moodleform_mod $formwrapper, MoodleQuickForm $mform, int $cmid): void {
        ...
        $mform->addElement("html", "<fieldset class='proctoring-block'><legend>" . get_string("legend", "proctoringpolicy_contract") . "</legend>");

        $mform->addElement("advcheckbox", "kopere_policy_contract_enabled", get_string("enabled_cm", "proctoringpolicy_contract"));
        $mform->setType("kopere_policy_contract_enabled", PARAM_INT);
        $mform->setDefault("kopere_policy_contract_enabled", 0);
