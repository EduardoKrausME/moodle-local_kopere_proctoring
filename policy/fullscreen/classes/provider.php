...
class provider implements policy_interface {
    ...
    public static function add_admin_settings(admin_settingpage $settings): void {
        ...
        $settings->add(
            new admin_setting_confightmleditor(
                ...
                get_string("message_default_text", "proctoringpolicy_fullscreen")
            )
        );
    }
    ...
    public static function add_module_form(moodleform_mod $formwrapper, MoodleQuickForm $mform, int $cmid): void {
        ...
        $mform->addElement("html", "<fieldset class='proctoring-block'><legend>" . get_string("legend", "proctoringpolicy_fullscreen") . "</legend>");

        $mform->addElement("advcheckbox", "kopere_policy_fullscreen_enabled", get_string("enabled_cm", "proctoringpolicy_fullscreen"));
        $mform->setType("kopere_policy_fullscreen_enabled", PARAM_INT);
        $mform->setDefault("kopere_policy_fullscreen_enabled", 0);
        ...
        $mform->addElement("static", "kopere_policy_fullscreen_limit_desc", "", get_string("limit_cm_desc", "proctoringpolicy_fullscreen"));
        ...
        $mform->addElement("text", "kopere_policy_fullscreen_limit", get_string("limit_cm", "proctoringpolicy_fullscreen"), ["size" => 10]);
        ...
        $mform->addElement("static", "kopere_policy_fullscreen_message_desc", "", get_string("message_cm_desc", "proctoringpolicy_fullscreen"));
    ...
    public static function get_js_config(int $cmid, int $attemptid): array {
        $enabled = cm_config::get("fullscreen", "enabled", $cmid, 0);
        if (!$enabled) {
            return [];
        }

        return [
            "limit" => cm_config::get("fullscreen", "limit", $cmid, 2),
