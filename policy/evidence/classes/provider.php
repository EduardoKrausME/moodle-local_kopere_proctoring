...
class provider implements policy_interface {
    ...
    public static function add_admin_settings(admin_settingpage $settings): void {
        ...
        $settings->add(
            new admin_setting_configtext(
                "proctoringpolicy_evidence/filearea_default",
                get_string("filearea_default", "proctoringpolicy_evidence"),
                get_string("filearea_default_desc", "proctoringpolicy_evidence"),
                "proctoring_evidence",
                PARAM_ALPHAEXT
            )
        );
    ...
    public static function add_module_form(moodleform_mod $formwrapper, MoodleQuickForm $mform, int $cmid): void {
        ...
        $mform->addElement("html", "<fieldset class='proctoring-block'><legend>" . get_string("legend", "proctoringpolicy_evidence") . "</legend>");

        $mform->addElement("advcheckbox", "kopere_policy_evidence_enabled", get_string("enabled_cm", "proctoringpolicy_evidence"));
        $mform->setType("kopere_policy_evidence_enabled", PARAM_INT);
        $mform->setDefault("kopere_policy_evidence_enabled", 0);
        $mform->hideIf("kopere_policy_evidence_enabled", "kopere_proctoring_enabled", "eq", "0");

        $mform->addElement("text", "kopere_policy_evidence_retention", get_string("retention_cm", "proctoringpolicy_evidence"), ["size" => 10]);
        ...
        $mform->addElement("text", "kopere_policy_evidence_maxfiles", get_string("maxfiles_cm", "proctoringpolicy_evidence"), ["size" => 10]);
        ...
        $mform->addElement("advcheckbox", "kopere_policy_evidence_allowdownload", get_string("allowdownload_cm", "proctoringpolicy_evidence"));
    ...
    public static function get_effective_cm_config(int $cmid): array {
        ...
        $globalmaxfiles = get_config("proctoringpolicy_evidence", "maxfiles_default");
        $globalallowdownload = get_config("proctoringpolicy_evidence", "allowdownload_default");
        $filearea = get_config("proctoringpolicy_evidence", "filearea_default");
