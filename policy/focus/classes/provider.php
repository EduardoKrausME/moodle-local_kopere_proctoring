...
class provider implements policy_interface {
    ...
    public static function add_admin_settings(admin_settingpage $settings): void {
        ...
        $settings->add(
            new admin_setting_heading(
                ...
                get_string("settings_heading", "proctoringpolicy_focus"),
                get_string("settings_heading_desc", "proctoringpolicy_focus")
            )
        );
        ...
        $settings->add(
            new admin_setting_configtext(
                ...
                get_string("settings_limit_default", "proctoringpolicy_focus"),
                get_string("settings_limit_default_desc", "proctoringpolicy_focus"),
                3,
                PARAM_INT
            )
        );
        ...
        $settings->add(
            new admin_setting_confightmleditor(
                ...
                get_string("settings_message_default", "proctoringpolicy_focus"),
                get_string("settings_message_default_desc", "proctoringpolicy_focus"),
                get_string("settings_message_default_value", "proctoringpolicy_focus")
            )
        );
    }
    ...
    public static function add_module_form(moodleform_mod $formwrapper, MoodleQuickForm $mform, int $cmid): void {
        ...
        $mform->addElement("html", "<fieldset class='proctoring-block'><legend>" .
            get_string("form_legend", "proctoringpolicy_focus") . "</legend>");

        // Enable checkbox.
        $mform->addElement("advcheckbox", "kopere_policy_focus_enabled",
            get_string("form_enabled_label", "proctoringpolicy_focus")
        );
        $mform->setType("kopere_policy_focus_enabled", PARAM_INT);
        $mform->setDefault("kopere_policy_focus_enabled", 0);
    ...
    public static function get_js_config(int $cmid, int $attemptid): array {
        $enabled = cm_config::get("focus", "enabled", $cmid, 0);
        if (!$enabled) {
            return [];
        }
